...
comes as an object of class `MSnSet`[^msnset2], which you can convert
to a `SummarizedExperiment` as shown below. Familiarise yourself with
its experimental design stored in the *colData* slot.

[^msnset2]: The [`MSnSet`
    class](https://lgatto.github.io/MSnbase/reference/MSnSet-class.html)
    is similar to the `SummarizedExperiment` class and can be
    converted to a `SummarizedExperiment` with `as(x,
...
remap the `r ncol(mulvey2015)` samples from a `r
nrow(mulvey2015)`-dimensional space into 2, or possibly 3
dimensions. This will require to transpose the data matrix before
...
sequencing. They do not aim reduce the dimensions according to
variance, as PCA, but focus on preserve neighbourhood between samples
in a lower-dimension embedding. However, this neighbourhood is only
maintained for close samples: close points in the lower-dimension
embedding are also close in high dimensions, while it isn't possible
to make any conclusion for distant points in low dimensions.
