...
The library composition might also be different accross samples. To
...
As for SummarizedExperiments (see chapter 3):
...
Note that you can also create a `DESeqDataSet` object from a
`SummarizedExperiment` (named `se` below) with by passing it to the
constructor and specifying a design formula:
...
Pick a gene highly expressed and a gene lowly expressed and inspect the effect
of the rlog transformation
...
[^log2C]: DESeq2 offers further options to shrink LFC estimates. This is useful for
ranking and visualization (see [DESeq2 vignette](https://bioconductor.org/packages/release/bioc/vignettes/DESeq2/inst/doc/DESeq2.html)
...
of genes which will have a significant padjusted value.
...
# All genes now have a padj value
...
the pvalues behaviour is now much nicer!
...
more informative.  Bioconductor's `r BiocStyle::Biocpkg("biomaRt")`
package can help with mapping various ID schemes to each other.
...
```{r, echo = FALSE}
ensembl_to_geneName <- readRDS(file = "./data/ensembl_to_geneName.rds")
```
...
Below, we store these results and the `DESeqDataSet` object for in the
next chapter.
...
The [*A guide to creating design matrices for gene expression
...
up design and contrast matrices. See also [this
chapter](https://uclouvain-cbio.github.io/WSBIM2122/sec-design.html)
